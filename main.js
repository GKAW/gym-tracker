...
var TrackerPlugin = class extends import_obsidian.Plugin {
  ...
  isWorkoutTableLine(line) {
    ...
    const isHeaderRow = cells.every(
      (cell, index) => cell.toLowerCase() === expectedHeaders[index].toLowerCase()
    );
    if (isHeaderRow) {
      return true;
    }
  ...
  findTableStart(editor, currentLine) {
  ...
  findTableEnd(editor, currentLine) {
  ...
  autoFillPreviousData(editor, lineNum, exerciseName) {
    ...
    if (cells.length >= 7) {
      ...
      cells[4] = ` ${lastData.reps} `;
      cells[5] = ` ${lastData.weight} `;
      cells[6] = ` ${lastData.date} `;
  ...
  saveExerciseFromTable(exercise, repsStr, weightStr) {
    if (repsStr.trim() && weightStr.trim()) {
      const lastData = this.getLastExerciseData(exercise);
      const today = new Date().toISOString().split("T")[0];
      if (!lastData || lastData.date !== today || lastData.reps !== repsStr || lastData.weight !== weightStr) {
...
<path d="M15.9718 2.07053V8.02821"/>
    ...
    (0, import_obsidian.addIcon)("dumbbell-regular", dumbbellRegularSvg);
    (0, import_obsidian.addIcon)("dumbbell-plus", dumbbellPlusSvg);
  }
  addWorkoutTable() {
    const activeView = this.app.workspace.getActiveViewOfType(import_obsidian.MarkdownView);
    if (!activeView) {
...
`;
  ...
  addWorkoutTableFromTemplate(template) {
    const activeView = this.app.workspace.getActiveViewOfType(import_obsidian.MarkdownView);
    if (!activeView) {
      return;
    }
    ...
    const time = now.toTimeString().split(" ")[0].slice(0, 5);
    let workoutTable = `
## \u{1F4AA} ${template.name} -- ${date}  ${time}
...
`;
  ...
  async addExerciseToHistory(exercise, reps, weight) {
  ...
  async addExerciseRow(exerciseName, reps, weight) {
    const activeView = this.app.workspace.getActiveViewOfType(import_obsidian.MarkdownView);
    if (!activeView) {
      return;
    }
...
`;
  ...
  async openOrUpdateDashboard() {
    ...
    if (dashboardFile && dashboardFile instanceof import_obsidian.TFile) {
  ...
  async createDashboard(path) {
    const content = this.generateDashboardContent();
    await this.app.vault.create(path, content);
  }
  async updateDashboard(file) {
...
> [!tip] \u{1F504} [Update Data](obsidian://advanced-uri?vault=${encodeURIComponent(this.app.vault.getName())}&commandid=tracker%3Aopen-dashboard)
...
`;
  ...
  updateDashboardContent(existingContent) {
  ...
  generateCombinedChart(exercises, dailyMaxData) {
...
`;
  ...
  generateDataTable(dailyMaxData, exercises) {
...
var ExerciseModal = class extends import_obsidian.Modal {
...
var TemplateEditModal = class extends import_obsidian.Modal {
  ...
  onOpen() {
    ...
    this.nameInput = nameDiv.createEl("input", {
      ...
      cls: "form-input",
      value: ((_a = this.template) == null ? void 0 : _a.name) || ""
    });
  ...
  async saveTemplate() {
    ...
    if (!name || !exercisesText) {
      new import_obsidian.Notice("Please fill in both template name and exercises");
      return;
    }
    const exercises = exercisesText.split("\n").map((line) => line.trim()).filter((line) => line.length > 0);
    if (exercises.length === 0) {
      new import_obsidian.Notice("Please add at least one exercise");
      return;
    }
    const newTemplate = {
      name,
    ...
    new import_obsidian.Notice(`Template "${name}" ${this.template ? "updated" : "created"}`);
    this.onSave();
    this.close();
  }
  onClose() {
...
var TrackerSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(dashboardDiv).setName("Dashboard file path").setDesc("Path where the progress dashboard will be created").addText((text) => text.setPlaceholder("_Workout Dashboard.md").setValue(this.plugin.settings.dashboardPath).onChange(async (value) => {
    ...
    initDashboardButton.addEventListener("click", async () => {
      ...
      new import_obsidian.Notice("Dashboard initialized");
    });
    containerEl.createEl("hr");
    ...
    if (exerciseCount > 0) {
      ...
      clearButton.addEventListener("click", async () => {
        if (confirmationInput.value === "CLEARALLHISTORY") {
          ...
          await this.plugin.saveSettings();
          new import_obsidian.Notice("Exercise history cleared");
          this.display();
        }
      });
    }
